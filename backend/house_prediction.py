...
def preprocess_student_data(path):
    ...
    scaler = MinMaxScaler()
    student_df["count_normalized"] = scaler.fit_transform(
        student_df[["count"]])
...
def create_student_prediction(student_df, scaler, target, plot = False):
    ...
    result_df = pd.DataFrame(
        {"year": new_X.squeeze(), "number_of_students": unscaled_y_pred}
    )
...
def preprocess_house_data(path):
    ...
    columns_to_drop = ['Suburb', 'Address', 'Method', 'SellerG', 'Distance', 'Bedroom2', 'CouncilArea',
                       'Lattitude', 'Longtitude', 'Regionname', 'Propertycount', 'YearBuilt', 'BuildingArea']
    ...
    housing_df_cleaned[['Car']] = housing_df_cleaned[['Car']].fillna(
        0)
    ...
    housing_df_cleaned['Date'] = pd.to_datetime(
        housing_df_cleaned['Date'], format="%d/%m/%Y")
    ...
    housing_df_cleaned['Postcode'] = housing_df_cleaned['Postcode'].astype('category')

    # Integrating the Student Model
    student_df = pd.read_csv(PREDICTION_PATH)

    # Merging the 2 files
    house_stud_df = housing_df_cleaned.merge(
        student_df, left_on='Year', right_on='year', how='left')
    
    # Dropping the duplicate "Year" Column
    house_stud_df = house_stud_df.drop(columns=['year'])

    # List of numerical columns to normalize
    columns_to_normalize = ['Rooms', 'Bathroom', 'Car', 'Landsize', 'Year', 'number_of_students']

    # Initialize the MinMaxScaler
    house_scaler = MinMaxScaler()

    # Fit the scaler to the numerical columns and transform them
    house_stud_df[columns_to_normalize] = house_scaler.fit_transform(house_stud_df[columns_to_normalize])
...
def train_house_model(house_stud_df, metrics):
    ...
    X_train, X_valid, y_train, y_valid = train_test_split(house_X, house_y, test_size=0.2, random_state=42)
...
def predict_house(features, plot, metrics):
    ...
    create_student_prediction(student_data, student_scaler, plot=plot, target=target)
    ...
    pred_features['number_of_students'] = [pred_student.loc[pred_student['year'] == features['Year'], 'number_of_students'].values[0]]
    ...
    columns_to_normalize = ['Rooms', 'Bathroom', 'Car', 'Landsize', 'Year', 'number_of_students']
    pred_features[columns_to_normalize] = house_scaler.transform(pred_features[columns_to_normalize])
